...
A deterministic diagnostic over 3360 audit trials shows the failure mode: aggregate robustness retains 0.614 mean task success while still producing 1 hidden failure cases, whereas a stagewise audit reduces the mean measured invariance gap from 0.173 to 0.048 and makes collapse locations explicit.
...
It contains 1200 rows in \texttt{docs/related\_work\_matrix.csv}, with the top-300 subset summarized in \texttt{docs/analysis\_summary.json}.
...
The stagewise audit changes the measurement target: it records each collapse as soon as it appears and reduces the mean measured gap to 0.048 by forcing the evaluation to account for the embodied stage where symmetry is lost.
